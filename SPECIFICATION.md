...
A zero-dependency virtual machine executing the compiled plan. Nine roles in
lifecycle order (`docs/next-project/seeai.md` §6): **host/** (the package
entry point, generated by the compiler's packaging phase; contract README
until S7), **loader/** (plan load and the resource contract; contract README
until S7, the code is inside dispatcher/update_engine), then the directories
below, with **gating/** (the accept-or-refuse decision) and **profiler/**
(instrumentation and the runtime screen) likewise contract READMEs until S7
extracts them from the dispatcher. Subsystems with code today:
...
| SEEU (update plan) | `"SEEU"` | v18, oldest-readable v4 | written by `compiler/backend/*` + driver; read/validated by `runtime/verifier` + `runtime/dispatcher`; disassembled by `seeml-seeu-dump` |
