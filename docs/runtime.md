...
SeeAI's runtime (`runtime/`) is a virtual machine whose instruction set happens to be *training*. It is the zero-dependency half of the product: the code vendored into every emitted package and executed on the device. No framework, no allocator churn, no graph library — because the compiler ([compiler.md](compiler.md)) already decided everything. What remains on the device is deliberately boring: one allocation, one thread pool, three fixed instruction streams, and a dispatch loop.
...
Would you install a software update that made your phone worse? Neither would SeeAI. If a validation split exists, the engine runs the **eval program** — the compiler's snapshot of the adapted forward pass plus loss, *before* autodiff ever touched the graph — over the held-out set before training and again after (rewinding the dataset so both passes see the identical sample sequence). The plan's batch shape is fixed, so a validation set that is not a whole number of batches ends in a batch whose last rows wrap around to samples already scored. Since G13 (#97) those duplicates do not count: for a plan whose loss is a lone cross-entropy or MSE, the final batch's per-row losses are recomputed from the loss instruction's own operands (the probabilities or the prediction and target), with the kernel's row expression, over its real rows only, so the reported loss is exactly the mean over the set's real samples. A composite loss (xent + KL, pure KL) keeps no per-row values, so its final batch is weighted by its real-row count instead; the duplicates still shape that batch's mean. A set of whole batches reports the plain mean of the batch losses, bit for bit as before. The update is accepted only if `val_final < val_initial`, strictly — and since E9 (#92) `val_final` scores the **best evaluated state**, not the endpoint. With a validation split the engine also evaluates every `eval_every` steps (the device driver's default is a tenth of the run) and keeps a copy of the persistent segment at the best held-out loss seen, the source model's own score being the bar to beat; when training ends that copy is what the arena holds, so merge and commit ship it. On the small corpora an on-device update trains on, the held-out loss bottoms out early and climbs: an endpoint that is still below step 0 used to be committed while a better state had been thrown away. The periodic evaluation writes transients only and the training program writes every transient it reads, so it changes no training bit; `--patience K` ends the run after K evaluations without a new best. `eval_every = 0` is the old loop: two evaluations, the last state committed. Without a validation split, the fallback compares the average training loss over the first and last few steps (a window of up to 20). The device stays untouched on rejection — exit code 3, and the caller can decide `--force` is warranted.
...
Floating-point addition is not associative: `(a + b) + c` and `a + (b + c)` can differ in the last bit. So if you split a sum across threads and combine "whenever threads finish," your result depends on scheduling — run twice, get two answers. SeeAI forbids this categorically, with one policy (`kernel_policy.h` + `source/parallel/`):
...
### The relaxed GEMM family — a permission, and what it buys

Plan v18 lets the compiler mark a frozen-weight GEMM `kFlagRelaxed` (`--precision certified-bf16`). The bit is a *permission*, never a demand: the exact kernel is always a valid execution of a relaxed instruction, and the CPU backend takes the permission only when the package was built with `SEEML_ACCELERATE` (the emitted `build.sh` sets it on Apple hosts for a relaxed plan), running an f32-weight GEMM on Accelerate's SGEMM — the AMX / SME matrix units the portable cores cannot reach, 2–3× their rate. The Metal backend takes it on every non-skinny, unbatched frozen-weight GEMM when its second kernel library built: Metal 4's tensor ops (`MetalPerformancePrimitives` `matmul2d`, the M5's per-core neural accelerators), two dispatches per GEMM — `k_to_bf16` rounds the activation to bfloat16 (round to nearest, ties to even, the rounding `tool/frontier_exec.py` models) into a backend-owned scratch, then the 64×32 tensor-op tile multiplies it against the *exact* stored weight (int8 levels, bf16, or f32) into f32 accumulators and applies the epilogue (the per-tensor or per-column int8 scale, bias, activation, addend) on the cooperative destination tensor. An NT int8 GEMM's reduction-index scales are folded into the activation before the rounding. The op fixes its reduction order, so a relaxed dispatch is bitwise run-to-run on its device like every other kernel here; its bits differ from the simdgroup kernels' and the CPU's by about one bf16 rounding of the activation, which is what the numerics certificate the plan must carry (P3) prices. Where Metal 4 is absent (before macOS 26) the backend says so in its device line and runs relaxed instructions on the exact kernels.
...
Shuffling poses a question worth dwelling on: how do you shuffle *reproducibly, on every platform*? Not with `std::shuffle` — the standard deliberately doesn't pin down distribution algorithms across implementations. SeeAI uses two classical pieces:
