...
- **BlockedNN** computes `C += α·A@B` with B row-major, tiled K-deep and N-wide by the plan header's GEMM tiles (v11 — the offline tuner's measured choice for the host, or the defaults of 64 and 256; throughput only, never bits) so the active B panel stays cache-resident; the inner loop is unrolled 4-wide over k — four independent multiply chains, four fewer C round-trips per pass.
