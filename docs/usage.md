...
The exporter accepts an `nn.Sequential` of `Linear`, `ReLU`, `GELU`, `SiLU`, and `LayerNorm` modules — anything else is a loud `ValueError`, not a silent skip. The exporter also refuses a model the device would compute differently (P7, #96): the device's GELU is the tanh approximation, so a plain `nn.GELU()` — PyTorch's default, the erf form — is an error that names the fix (`nn.GELU(approximate='tanh')`, or `export_smf(..., gelu_tanh_ok=True)` to accept a drift of up to 4.7e-4 per activation); and a `LayerNorm`'s own `eps` is written into the file (SMF v6) whenever it is not 1e-5, as is a Hugging Face checkpoint's `rms_norm_eps` under `--hf`. One detail worth knowing so the format makes sense later: PyTorch stores a `Linear`'s weight as `[out, in]`, but SMF's `MatMul(x, W)` wants `[in, out]`, so the exporter transposes on the way out. Labels: pass int32 class indices for cross-entropy, dense float vectors for MSE, or `None` for a distillation corpus (the teacher provides the targets).
...
**How big.** `--quantize-base` stores eligible frozen weights as int8 in the plan (4× smaller, dequantization fused into the GEMM for free); `--bf16-base` stores them as bfloat16 instead (2× smaller, float32's exponent range with 8 bits of mantissa, widened exactly inside the GEMM — the gentler choice for outlier-heavy weights; the two flags are mutually exclusive). Either way, because commit patches the *original file's* floats, storage rounding never reaches the committed model — and since E12 (#95) the gate measures that committed model: a plan compiled from a model file evaluates the student's f32 weights straight from the file at run time, so the validation losses the gate compares (and the best-state choice) are the shipped model's, not the narrow copies' (`validation_scores: "shipped"` in both reports). int8 scales are per output column, which keeps outlier-heavy projections' bulk at full resolution; teacher weights stay f32. `--steps` (default 1000) is the default optimizer-step count baked into the plan (the device may override it). `--grad-accum G` (default 1) compiles at the micro-batch `--data-batch` and accumulates `G` micro-batch gradients per optimizer step: the effective batch is `data-batch × G` while activation memory stays that of one micro-batch, at the cost of one gradient-sized accumulator per adapter in the persistent segment (the compile report lists `effective_batch` and `step_instructions`).

**How precise.** `--precision f32` (the default) is the reference: every kernel exact, every backend's bits its own. `--precision certified-bf16` (plan v18) marks the frozen-weight GEMMs of the train and step programs — the forward and dX products through the base (and teacher) weights, in f32, int8 or bf16 storage — as *relaxed*: a permission for the backend to run them on a certified relaxed kernel. On Metal that is the tensor-op family (the M5's neural accelerators): the f32 activations are rounded to bfloat16 once at the GEMM input and multiplied against the exact stored weight into f32 accumulators, 2–2.5× the exact kernels' rate. On the CPU, an f32-base plan's relaxed GEMMs run on Accelerate's SGEMM (the AMX / SME matrix units) when the package is built on an Apple host (`SEEML_NO_ACCELERATE=1` keeps the portable kernels); int8 and bf16 bases stay on the portable widening kernels. The adapter GEMMs, attention, norms, losses and the optimizer are exact everywhere, the eval program scores what ships in exact arithmetic, and a relaxed kernel is still bitwise run-to-run on its device — but its bits are not the reference's, so *no certificate, no relaxed plan*: `tool/pack_update.py` refuses to package the plan until `tool/certify_numerics.py certify pkg/ --corpus corpus.sds [--source model.smf] --max-loss-deviation 1e-2` has priced the `gemm.relaxed` site on the plan's own operands and granted (the bf16 rounding is 2⁻⁹ relative per element, so the site's default tolerance is 2⁻⁷ and a short run's loss moves ~1e-3). The compile report carries `precision` and `relaxed_gemms`; `seeml-seeu-dump` shows the bit as `relaxed`.
