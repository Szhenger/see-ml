...
constexpr const char* kUsage =
    ...
    " [--eval-every N] [--patience K] [--force] [--help]\n"
    "  --eval-every N: with a validation split, evaluate every N steps and\n"
    "  commit the BEST evaluated state, not the last (default: steps/10,\n"
    "  at least 1; 0 evaluates only before and after and commits the last).\n"
    "  --patience K: stop after K consecutive evaluations without a new best\n"
    "  (default 0: never).\n"
...
int main(int argc, char** argv) {
  ...
  // The best-state gate (E9): "auto" resolves inside the engine, against
  // the step count the run actually has (a resume knows its remainder).
  const std::string eval_every_text = Arg(argc, argv, "--eval-every", "auto");
  uint64_t eval_every = seeml::update_rt::TrainOptions::kEvalEveryAuto;
  uint64_t patience = 0;
  if ((eval_every_text != "auto" && !ParseU64(eval_every_text.c_str(), &eval_every)) ||
      !ParseU64(Arg(argc, argv, "--patience", "0"), &patience)) {
    std::fprintf(stderr, "invalid --eval-every / --patience\n");
    return 2;
  }
  if (patience > 0 && (eval_every == 0 || val_frac <= 0.0)) {
    std::fprintf(stderr,
                 "model_update: --patience needs periodic evaluation "
                 "(--eval-every > 0) and a validation split (--val-frac > 0)\n");
    return 2;
  }
  ...
  // A resume is bound to the first run's shuffle seed and split (SEKP v5):
  // a different one would train on that run's validation rows. Refused
  // here as the usage error it is, before the engine loads anything; the
  // engine refuses again for callers that skip this check.
  if (options.resume && !options.checkpoint_path.empty()) {
    auto peek = seeml::update_rt::PeekCheckpointRecord(options.checkpoint_path);
    if (peek && peek->has_binding &&
        (peek->shuffle_origin != dataset->shuffle_origin() ||
         peek->train_samples != dataset->num_samples() ||
         peek->val_samples != (val_set ? val_set->num_samples() : 0))) {
      std::fprintf(stderr,
  ...
  if (report->best_tracked)
    std::fprintf(stderr,
                 "seeml-update: best state at step %llu (validation loss "
                 "%.6f); last state %.6f; %llu periodic evaluation(s)%s\n",
                 (unsigned long long)report->best_step, report->val_final_loss,
                 report->val_last_loss,
                 (unsigned long long)report->evaluations,
                 report->stopped_by_patience ? "; stopped: out of patience"
                                             : "");
  ...
  auto write_report = [&](bool committed, const char* error) {
    ...
    if (report->best_tracked) {
      ...
      std::fprintf(f,
                   "  \"best_state\": {\"step\": %llu, \"last_loss\": ",
                   (unsigned long long)report->best_step);
      num(report->val_last_loss);
      std::fprintf(f,
                   ", \"eval_every\": %llu, \"patience\": %llu, "
                   "\"evaluations\": %llu, \"stopped_by_patience\": %s},\n",
                   (unsigned long long)report->eval_every,
                   (unsigned long long)report->patience,
                   (unsigned long long)report->evaluations,
                   report->stopped_by_patience ? "true" : "false");
    }
...
constexpr const char* kVendoredSources[] = {
    "runtime/dispatcher/update_engine.h",     "runtime/dispatcher/update_engine.cc",
    "runtime/dispatcher/contract.h",          "runtime/dispatcher/contract.cc",
    ...
    "runtime/pipeline/dataset.h",           "runtime/pipeline/dataset.cc",
    "runtime/pipeline/batch_pipeline.h",    "runtime/pipeline/batch_pipeline.cc",
    "runtime/verifier/plan_validator.h", "runtime/verifier/plan_validator.cc",
    "runtime/storage/durable_io.h",     "runtime/storage/durable_io.cc",
    "runtime/storage/checkpoint_format.h",
    "runtime/storage/checkpoint.h",     "runtime/storage/checkpoint.cc",
...
std::string BuildScript(bool relaxed_plan) {
  std::string s;
  s += "#!/bin/sh\n";
  ...
  // Relaxed plans (v18): the CPU backend may run the frozen-weight f32
  // GEMMs on Accelerate — the AMX / SME matrix units — on Apple hosts. An
  // exact plan's script never mentions the framework, so the zero-
  // dependency promise holds by default and the exception is visible.
  s += "ACCEL_FLAGS=\"\"\n";
  s += "ACCEL_LDFLAGS=\"\"\n";
  if (relaxed_plan) {
    s += "# This plan carries relaxed GEMMs (--precision certified-bf16, plan\n";
    s += "# v18, certified at packaging): on Apple hosts the CPU backend runs\n";
    s += "# their f32-weight forms on Accelerate's SGEMM (the AMX / SME matrix\n";
    s += "# units). SEEML_NO_ACCELERATE=1 keeps every kernel portable.\n";
    s += "if [ \"$(uname -s)\" = \"Darwin\" ] && [ -z \"${SEEML_NO_ACCELERATE:-}\" ]; then\n";
    s += "  ACCEL_FLAGS=\"-DSEEML_ACCELERATE\"\n";
    s += "  ACCEL_LDFLAGS=\"-framework Accelerate\"\n";
    s += "fi\n";
  }
  s += "FLAGS=\"$FLAGS $ACCEL_FLAGS\"\n";
  ...
  s += "for unit in executor/gemm executor/elementwise executor/activation "
       ...
       "pipeline/dataset pipeline/batch_pipeline verifier/plan_validator "
       "storage/durable_io storage/checkpoint dispatcher/contract "
       "dispatcher/update_engine; do\n";
  ...
  s += "$CXX $FLAGS update_main.o update_plan_embedded.o update_engine.o "
       ...
       "$METAL_LDFLAGS $ACCEL_LDFLAGS -o model_update\n";
...
std::expected<EmitPaths, std::string> EmitNativePackage(
  ...
  if (auto r = WriteFile(paths.build_script,
                         BuildScript(options.relaxed_plan),
                         /*executable=*/true);
      !r)
    return std::unexpected(r.error());
